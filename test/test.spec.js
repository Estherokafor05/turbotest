...
test("Extract Stack Overflow Questions", async ({ page }) => {
  ...
  await page.fill('input[role="combobox"][placeholder="e.g. javascript or python"]', "javascript");
  ...
  console.log(`Extracted ${questionsData.length} questions. Data saved to questions.csv`);
...
async function extractQuestions(page) {
  return await page.evaluate(() => {
    ...
    questionElements.forEach((questionElement) => {
      const titleElement = questionElement.querySelector("h3.s-post-summary--content-title a");
      const tags = Array.from(questionElement.querySelectorAll(".s-post-summary--meta-tags a"))
        .map((tag) => tag.innerText.trim());
      const votesElement = questionElement.querySelector(".s-post-summary--stats-item__emphasized .s-post-summary--stats-item-number");
      const timestampElement = questionElement.querySelector(".s-user-card--time span[title]");

      questions.push({
        title: titleElement ? titleElement.innerText.trim() : "No title",
        tags: tags.join(", "),
        votes: votesElement ? votesElement.innerText.trim() : "0",
        timestamp: timestampElement ? timestampElement.getAttribute("title") : "No timestamp",
        userId: questionElement.querySelector(".s-user-card--avatar")?.getAttribute("data-user-id") || "Unknown",
      });
    });
    return questions;
  });
